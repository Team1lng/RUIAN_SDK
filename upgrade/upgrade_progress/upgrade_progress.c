...
static void upgrade_fb_progress_display(void)
  {
      ...
      int w = h_size / 6 * 4 - 6;
      int h = 10;
      int fill_w = upgrade_total ? (upgrade_count * w / upgrade_total) : w;

      /* clear progress area */
      fb_rect_draw(x, y, w, h, 0x00, 0x00, 0x00);

      if (fill_w > 0) {
          fb_rect_draw(x, y, fill_w, h,
              (char)(process_color >> 16),
              (char)(process_color >> 8),
              (char)(process_color));
      }
  }
...
{
	printf("start run display process \n");
    if(argc != 3)
    {
        printf("请输入参数，如：[./upgrade_progress 10 1]\n");
        return 0;
    }
	printf("start fb init \n");
    fb_init();
	printf("fb init success\n");
	...
	while(1)
	{
		...
		else
		{
			...
			if(proc_value > upgrade_total || proc_value < 0)
			{
				continue;
			}
