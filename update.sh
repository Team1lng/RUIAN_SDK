...
killall -9 ftpd
kill_proc "/usr"
                                                                #关闭所有ps里面带 "/usr"关键字的进程kill掉
if [ -x "$DIR_UPDATE/upgrade_progress" ]; then
    "$DIR_UPDATE/upgrade_progress" 11 0 &
elif [ -x /tmp/upgrade_progress ]; then
    /tmp/upgrade_progress 11 0 &
else
    /usr/sbin/upgrade_progress 11 0 &
fi
...
if [ "$DD_UPDATE" = 1 ];then
    $CMD_ECHO "############ update success, no reboot, start blink progress bar #############"
    touch /app/data/update_flag #用来标志是重启后是否需要再次升级
    
    # 无限循环：进度条在满进度(11)和空进度(0)之间切换，实现闪烁
    while true; do
        echo 11 > /tmp/proc_value  # 满进度
        sleep 0.5                  # 停留0.5秒
        echo 0 > /tmp/proc_value   # 空进度
        sleep 0.5                  # 停留0.5秒
    done
fi
