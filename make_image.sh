...
get_config_info()
{
	...
	PARTITION_TYPE=`awk -F= '/^PARTITION_TYPE/{gsub(" ","",$2); printf $2}' config.mk`
...
process_flash_scripts()
{
    if test $FLASH_TYPE = "NOR" ; then
        cp ./scripts/flash/new/rc.local.nor rootfs/etc/init.d/rc.local
        cp ./scripts/flash/new/update.nor.sh rootfs/sbin/update.sh
    elif test $FLASH_TYPE = "NAND" ; then
        cp ./scripts/flash/old/rc.local.nand rootfs/etc/init.d/rc.local
        cp ./scripts/flash/old/update.nand.sh rootfs/sbin/update.sh
...
process_wifi_scripts()
{
    ...
    if test $WIFI_TYPE = "NO_WIFI" ; then
        ...
        cp scripts/wifi/hi3881.sh rootfs/usr/sbin/wifi_driver.sh
        cp -rf ./wifi/* rootfs/
    else
...
make_image()
{
    # 根据选择的Flash类型制作烧录镜像
    if test $FLASH_TYPE = "NOR" ; then
        if test $PARTITION_TYPE = "NEW" ; then
            cd create_new
        else
            cd create_old
        fi
...
make_resource()
{
    if test $PARTITION_TYPE = "NEW" ; then
        rm -rf resource/*
        cp -rf resource_new/* resource/
    else
        rm -rf resource/*
        cp -rf resource_old/* resource/
    fi
}
