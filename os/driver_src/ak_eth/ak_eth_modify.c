...
static s32 ak_gmac_phy_init(struct net_device *netdev, gmac_device *gmacdev)
{
    ...
    else if (data_id1 == 0x0 && data_id2 == 0x109)
    {
        ...
        */
        ...
        /** 驱动能力*/
        ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x4001);
        ak_gmac_read_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1f, &data_ctl);
        data_ctl |= (0x01<<4)|(0x01<<5);
        ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x4001);
        ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1f, data_ctl);
        ...
        /*** 切换到lds mii寄存器下面 ***/
        ak_gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x100);
        ak_gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0x04);
       // data_ctl &= 0xFFFD;
       // ak_gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x100);
       // ak_gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x1f, data_ctl);
        ...
        ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x100);
        ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0x07);
        //data_ctl |= 0x02;
        //ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x100);
        //ak_gmac_write_phy_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase, 0x1f, data_ctl);
...
static void resolve_10M_idle_issue(gmac_device *gmacdev)
{
    u16 data;
    ak_gmac_read_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x2023, &data);
    printk(KERN_ERR "read ext reg 0x2023 is %x\n", data);
    ak_gmac_write_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x2023, data & 0xff7f);
    ak_gmac_read_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x2023, &data);
    printk(KERN_ERR "read ext reg 0x2023 is %x\n", data);
}

static void driving_ability_adjust(gmac_device *gmacdev)
{
    u16 data;
    ak_gmac_read_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x4001, &data);
    printk(KERN_ERR "read ext reg 0x4001 is %x\n", data);
    ak_gmac_write_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x4001, 0x3);
    ak_gmac_read_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x4001, &data);
    printk(KERN_ERR "read ext reg 0x4001 is %x\n", data);
}
...
static int ak_mac_ctrl_reinit(struct net_device *netdev)
{
    ...
    //    resolve_10M_idle_issue(gmacdev);
...
static s32 ak_gmac_check_phy_init(struct net_device *netdev, gmac_device *gmacdev)
{
    ...
    ak_gmac_read_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x71, &data_power);

    printk(KERN_ERR
           "before init address 0x71 data is 0x%x\n",
           data_power);
    // ak_gmac_write_phy_ext_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase,0x71,data_power|0x0003);

    // ak_gmac_read_phy_ext_reg((u32 *) gmacdev->MacBase, gmacdev->PhyBase,0x71,&data_power);

    // printk(KERN_ERR
    // "after_modify address 0x71 data is 0x%x\n",data_power);

  //  driving_ability_adjust(gmacdev);
    ...
    u16 data_status;
    ak_gmac_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x11, &data_status);
    printk(KERN_ERR
           "address 0x11 data is 0x%x\n",
           data_status);
    /*
     * Auto-Negotiation
    ...
    else if (data_id1 == 0x0 && data_id2 == 0x109)
    {
        ...
        u16 data_status;
        ak_gmac_read_phy_ext_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, 0x71, &data_status);
        printk(KERN_ERR
               "address 0x71 data is 0x%x\n",
               data_status);
