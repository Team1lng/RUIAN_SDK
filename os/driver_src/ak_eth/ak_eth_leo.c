...
static void rgb_pal_plck_enable(u32 regbase, int enable)
{
    u32 reg = (u32)regbase + 0xBC; // AK_VA_SYSCTRL;
    u32 regval = 0;
    if (enable)
    {
        printk("========1==========lcd pclk enable\n");

        return ;
        regval = __raw_readl((void *)reg);
        regval |= (0x1 << 8);
        __raw_writel(regval, (void *)reg);
        printk("=========2=========lcd pclk enable\n");
    }
    else
    {
        printk("=========1=========lcd pclk disable\n");
        regval = __raw_readl((void *)reg);
        regval &= ~(0x1 << 8);
        __raw_writel(regval, (void *)reg);
        printk("=========2=========lcd pclk disable\n");
    }
}
...
static s32 ak_gmac_check_phy_init(struct net_device *netdev, gmac_device *gmacdev)
{
    ...
    else if (data_id1 == 0x0 && data_id2 == 0x109)
    {
        ...
        rgb_pal_plck_enable(gmacdev->lcd_base, 0);
        lcd_pclk_enable = 1;
    ...
    if (lcd_pclk_enable)
    {
        rgb_pal_plck_enable(gmacdev->lcd_base, 1);
    }
...
static int ak_mac_probe(struct platform_device *pdev)
{
...
#ifdef LEO_RGB_LCD_PCLK_CTRL
    printk("============%s:%d======\n",__func__,__LINE__);
    pdev->id = of_alias_get_id(node, "lcdc");
    printk("============%s:%d======\n",__func__,__LINE__);
    res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
    if (!res)
    {
    ...
    priv->lcd_res = request_mem_region(res->start, resource_size(res), dev_name(&pdev->dev));
    printk("============%s:%d======\n",__func__,__LINE__);
    if (!priv->res)
    {
        dev_err(&pdev->dev, "Could not reserve memory region\n");
    ...
    printk("===========lcd base:%lu\n", priv->lcd_res->start);
    priv->lcd_base = ioremap( priv->lcd_res->start, resource_size(res));
    if (!priv->lcd_base)
    {
        dev_err(&pdev->dev, "Failed to ioremap ethernet registers\n");
        err = -EIO;
    ...
    printk("============%s:%d======\n",__func__,__LINE__);
    priv->gmacdev.lcd_base = (u32)priv->lcd_base;
    printk("============%s:%d======\n",__func__,__LINE__);
#endif
...
err_mac_init:
    iounmap(priv->base);
#ifdef LEO_RGB_LCD_PCLK_CTRL
     iounmap(priv->lcd_base);
#endif
err_ioremap:
    release_resource(priv->res);
#ifdef LEO_RGB_LCD_PCLK_CTRL
     release_resource(priv->lcd_res);
#endif
err_req_mem:
    free_netdev(netdev);
err_alloc_etherdev:
