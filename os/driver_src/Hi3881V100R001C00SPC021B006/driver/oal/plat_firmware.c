...
static hi_s32 exec_quit_type_cmd(firmware_mem_stru *firmware_mem)
{
    ...
    if (memset_s(firmware_mem->puc_send_cmd_buff, CMD_BUFF_LEN, 0, 8) != EOK)
    { /* 8: 清8个长度 */
        return -OAL_EFAIL;
    } /* buffer len 8 */
...
hi_s32 firmware_write_cfg(hi_u8 *key, hi_u8 *new_val, hi_u8 len)
{
    ...
    /* 校验新值的长度        */
    if (len > g_cus_cfg_cmd[idx].val_len - 3)
    { /* 3:预留的 ‘;’和“\r\n”的大小 */
    ...
    if (len < g_cus_cfg_cmd[idx].val_len - 2)
    {                   /* 2:回车换行预留 */
        buf[len] = ';'; /* 重置结束符 */
    }
    ...
    if (l_ret == g_cus_cfg_cmd[idx].val_len)
    { /* 返回值为写入字节数表示成功 */
        /* 更新缓存 */
...
static hi_s32 firmware_parse_cfg(hi_u8 *puc_cfg_info_buf, hi_s32 l_buf_len, hi_u32 ul_index)
{
    ...
    while ((i < g_st_cfg_info.al_count[ul_index]) && (flag < &puc_cfg_info_buf[l_len]))
    {
        ...
        *end = '\0';
        ...
        else if (cmd_type != ERROR_TYPE_CMD)
        { /* 正确的命令类型，增加 */
            ...
            if (memcpy_s(g_st_cfg_info.apst_cmd[ul_index][i].cmd_name, DOWNLOAD_CMD_LEN, cmd_name, DOWNLOAD_CMD_LEN) !=
                    EOK ||
                memcpy_s(g_st_cfg_info.apst_cmd[ul_index][i].cmd_para, DOWNLOAD_CMD_PARA_LEN, cmd_para,
...
hi_s32 firmware_download(hi_u32 ul_index)
{
    ...
    for (i = 0; i < g_st_cfg_info.al_count[ul_index]; i++)
    {
        ...
        {  
            ...
            if (ul_index == RAM_REG_TEST_CFG && ((!memcmp(puc_cmd_name, JUMP_CMD_KEYWORD, strlen(JUMP_CMD_KEYWORD))) && (g_ul_jump_cmd_result == CMD_JUMP_EXEC_RESULT_FAIL)))
            {
