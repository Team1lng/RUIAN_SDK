...
extern int atbm_Default_driver_configuration(struct atbm_common *hw_priv);
static int __atbm_sdio_lmac_restart(struct sbus_priv *self)
{
	...
	*/
	...
	ret = atbm_Default_driver_configuration(hw_priv);
...
static int atbm_sdio_adjust_wsm(struct sbus_priv *self,void *wsm)
{
	...
	if(hw_priv->chip_version >= ARES_A){
		...
		if (tx_len % (SDIO_BLOCK_SIZE) ) {
			tx_len -= (tx_len % (SDIO_BLOCK_SIZE) );
...
static struct sbus_ops atbm_sdio_sbus_ops = {
	...
	.sbus_adjust		   = atbm_sdio_adjust_wsm,
#if 0
			      ...
			      const struct sdio_device_id *id)
{
	...
	atbm_printk_err("atbm_sdio_probe : idVendor[%x] idProduct[%x] \n",id->vendor,id->device);
