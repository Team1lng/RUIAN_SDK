...
#endif
		{SIOCIWFIRSTPRIV + 0xb, IW_PRIV_TYPE_CHAR | 500,IW_PRIV_TYPE_CHAR|1024, "common"},
...
static int atbm_set_rf_low_power(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext);
...
extern void atbm_get_delta_gain_V2(char *srcData,int *allgain,int *bgain,int *gngain);
...
static int atbm_ioctl_get_wifi_temp(struct net_device *dev, struct iw_request_info *info,  union iwreq_data  *wrqu, char *extra);
static int atbm_ioctl_get_txrx_info(struct net_device *dev, struct iw_request_info *info,  union iwreq_data  *wrqu, char *extra);
//sstatic int atbm_ioctl_get_sta_info(struct net_device *dev, struct iw_request_info *info,  union iwreq_data  *wrqu, char *extra);
...
static int atbm_ioctl_get_sta_info(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra);
...
iwripv_common_cmd_t common_cmd[]={
	...
	{"setEfuse_5gdeltagain",20,atbm_ioctl_set_efuse,"effect 5g tx power"},
#ifdef CONFIG_ATBM_IWPRIV_USELESS
...
//	{"send_deauth",11,atbm_ioctl_send_deauth,"send brodcast deauth frame"},
	...
	{"get_chip_temp",13,atbm_ioctl_get_wifi_temp,"get chip current work temperature"},
...
#endif
	{"get_ap_info",11,atbm_ioctl_get_ap_info,"get ap info"},
	{"get_sta_info",12,atbm_ioctl_get_sta_info,"get sta info"},
	...
	{"rf_low_power",9,atbm_set_rf_low_power,"0:normal,1:set rf low power"},
	{NULL,0,NULL,NULL}
};
...
int fwcmd_common_deal(struct ieee80211_sub_if_data *sdata,char *extra,int len)
{
	...
	if((len > 12) && (!memcmp(extra,"set_adaptive",12))){
		...
		if(extra[4] == '0'){
			...
			//extra[4] = '1';
			atbm_wifi_cfo_set(extra[4]-'0');
		}
	}
	else if((len > 7)&&(!memcmp(extra, "set_dur", 7)))
	{
		for(i = 8;i < len - 1;i++)
		{
			dur_val = dur_val * 10 + (extra[i] - 0x30);
		}
		wsm_write_mib(sdata->local->hw.priv, WSM_MIB_ID_SET_RTS_DUR, &dur_val, sizeof(dur_val), 0);
	}
	return 0;
}
...
//	struct atbm_vif *vif;
	...
	*/
	if(len != 0)
		ATBM_WEXT_PROCESS_PARAMS(pos,len,low_power,atbm_accsii_to_int,false,exit,ret);
	
	ret = atbm_internal_start_rx(sdata,channel,is_40M,low_power);
...
static int atbm_ioctl_get_wifi_temp(struct net_device *dev, struct iw_request_info *info,  union iwreq_data  *wrqu, char *extra)
{
	...
	ret = atbm_get_temperature(hw_priv,(char *)&temp,sizeof(int));
	sprintf(extra,"\nchip temperature=%d\n",temp);
...
static int atbm_ioctl_get_txrx_info(struct net_device *dev, struct iw_request_info *info,  union iwreq_data  *wrqu, char *extra)
{
	...
	*/
	...
	list_for_each_entry_rcu(sta, &local->sta_list, list) {
		...
		if (sdata->vif.type == NL80211_IFTYPE_AP){
			sta_id = sta->sta.aid;
			atbm_printk_err("sta_id = %d \n",sta_id);
			wsm_write_mib(hw_priv, WSM_MIB_ID_RX_RATE_AVERAGE, &sta_id, sizeof(sta_id), priv->if_id);
		}
...
//		struct ieee80211_sta* sta;
		...
		wsm_write_mib(hw_priv, WSM_MIB_ID_GET_RATE, &sta_id, 1, priv->if_id);
#endif
		maxrate_id = atbm_internal_get_current_txrate(sdata,mac_addr);
	}else{

		maxrate_id = atbm_internal_get_current_txrate(sdata,NULL);
...
#endif	
	if(maxrate_id > 0){
		atbm_printk_always("rate: %d/10 Mbits/s\n", maxrate_id);
//	else
...
//	union iwreq_data *wdata = (union iwreq_data *)wrqu;
	...
	if ((ret = wsm_get_SIGMSTAR_256BITSEFUSE(hw_priv, &efuseBuff[0], efuse_len)) == 0){
	...
	*/
	if(extra){	
		memcpy(extra,efuseBuff,efuse_len);	
		wrqu->data.length = efuse_len;
	}
...
static int atbm_ioctl_set_SIGMSTAR_256BITSEFUSE(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	trueLen =((wrqu->data.length - strheadLen) > efuse_len) ? efuse_len:(wrqu->data.length - strheadLen);
	...
	if ((ret = wsm_set_SIGMSTAR_256BITSEFUSE(hw_priv, &efuseBuff[0], trueLen)) == 0)
	{
...
//	int index = 0;
	...
	else if(memcmp(cmd, "setEfuse_deltagain", 18) == 0)
	{
		...
		if(rxData < 0 || rxData > deltagainMax){
			atbm_printk_err("ERR!!!  efuse delta_gain1 range is 0~%d,input value is [%d] \n",deltagainMax,rxData);
			return -EINVAL;
			goto efuse_err;
		}
		//atbm_printk_err("%s %d\n", __func__, __LINE__);
		...
		if(rxData < 0 || rxData > deltagainMax){
			atbm_printk_err("ERR!!!  efuse delta_gain2 range is 0~%d,input value is [%d] \n",deltagainMax,rxData);
			return -EINVAL;
			goto efuse_err;
		}
		//atbm_printk_err("%s %d\n", __func__, __LINE__);
		...
		if(rxData < 0 || rxData > deltagainMax){
			atbm_printk_err("ERR!!!  efuse delta_gain3 range is 0~%d,input value is [%d] \n",deltagainMax,rxData);
			return -EINVAL;
			goto efuse_err;
		}
	...
	else if(memcmp(cmd, "setEfuse_5gdeltagain", 20) == 0)
	{
		...
		efuse_temp.delta_gain1_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain2_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain2_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain3_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain3_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain4_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain4_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain5_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain5_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain6_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain6_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain7_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain7_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain8_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain8_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain9_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain9_5g = rxData;
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if((rxData < 0) || (rxData > deltagainMax)){
			atbm_printk_err("invalid delta_gain10_5g:%d\n", rxData);
			goto efuse_err;
		}
		efuse_temp.delta_gain10_5g = rxData;
		
		CmdLine_GetHex(&pRxData, &writeEfuseFlag);
		set_deltagain_5g = 1;
		atbm_printk_err("set efuse data is delta_gain 5g[%d,%d,%d,%d,%d,%d,%d,%d,%d,%d]\n",
			efuse_temp.delta_gain1_5g,efuse_temp.delta_gain2_5g,efuse_temp.delta_gain3_5g,efuse_temp.delta_gain4_5g,efuse_temp.delta_gain5_5g,
			efuse_temp.delta_gain6_5g,efuse_temp.delta_gain7_5g,efuse_temp.delta_gain8_5g,efuse_temp.delta_gain9_5g,efuse_temp.delta_gain10_5g);
	}
	else if(memcmp(cmd, "setEfuse_tjroom", 15) == 0)
	{
		...
		if(rxData < 0 || rxData > 127){
			atbm_printk_err("ERR!!!  efuse_Tj_room range is 0~127,input value is [%d] \n",rxData);
			return -EINVAL;
			goto efuse_err;
		}
		CmdLine_GetHex(&pRxData, &writeEfuseFlag);
	...
	if(writeEfuseFlag)
	{
		...
		if(set_deltagain_5g){
			memset(writebuf, 0, sizeof(writebuf));
			sprintf(writebuf, "set_txpwr_5g,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",
			efuse_temp.delta_gain1_5g,efuse_temp.delta_gain2_5g,efuse_temp.delta_gain3_5g,efuse_temp.delta_gain4_5g,efuse_temp.delta_gain5_5g,
			efuse_temp.delta_gain6_5g,efuse_temp.delta_gain7_5g,efuse_temp.delta_gain8_5g,efuse_temp.delta_gain9_5g,efuse_temp.delta_gain10_5g);
...
static int atbm_ioctl_set_all_efuse(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	if(hw_priv->efuse.version == 0){
		if(hw_priv->chip_version == Mercurius)
			hw_priv->efuse.version = 2;
		else
...
static int atbm_ioctl_set_txpwr_by_file(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	{
		atbm_printk_init("param:\n%s\n",readbuf);		
		if(hw_priv->chip_version == Mercurius){
			atbm_get_delta_gain_V2(readbuf,deltagain,bgain,gngain);
		
			if(deltagain[13]==0)
				deltagain[13] = hw_priv->efuse.dcxo_trim;
			//2.4G deltagain and dcxo
			memset(readbuf, 0, sizeof(readbuf));
			sprintf(readbuf, "set_txpwr_and_dcxo,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",deltagain[0], deltagain[1], deltagain[2], deltagain[13],
				bgain[0], bgain[1], bgain[2],gngain[0], gngain[1], gngain[2]);
			atbm_printk_init("2.4G cmd: %s\n", readbuf);
			ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, readbuf, strlen(readbuf), if_id);
			//5G deltagain
			memset(readbuf, 0, sizeof(readbuf));
			sprintf(readbuf, "set_txpwr_5g,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",
			deltagain[3],deltagain[4],deltagain[5],deltagain[6],deltagain[7],deltagain[8],deltagain[9],deltagain[10],deltagain[11],deltagain[12],
			bgain[3],bgain[4],bgain[5],bgain[6],bgain[7],bgain[8],bgain[9],bgain[10],bgain[11],bgain[12],
			gngain[3],gngain[4],gngain[5],gngain[6],gngain[7],gngain[8],gngain[9],gngain[10],gngain[11],gngain[12]);
			atbm_printk_init("5G cmd: %s\n", readbuf);
		ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, readbuf, strlen(readbuf), if_id);
		}
		else{
...
static int atbm_set_rf_low_power(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	low_power = buff_data[strheadLen] - 0x30;

	ret = wsm_set_rf_low_power(hw_priv, &low_power, sizeof(low_power));
	if(ret != 0)
		atbm_printk_err("Set RF low power failed\n");
	else
		atbm_printk_err("Set RF low power success\n");
...
int atbm_ioctl_get_cfo_cali_data(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if(crystal_type==2)
		atbm_printk_always("crystal_type=2,%s ,rx_cfo:%d ppm,tx_cfo:%d ppm,cfo_ppm_has_init_flag:%d\n", "share crystal",cfo_val.rx_cfo,
		cfo_val.tx_cfo,cfo_val.cfo_ppm_has_init_flag);
	else
		atbm_printk_always("crystal_type=%d,%s,dcxo:%d ,rx_cfo:%d ppm,cfo_ppm_has_init_flag:%d\n", crystal_type,"independent crystal", dcxo,cfo_val.rx_cfo,cfo_val.cfo_ppm_has_init_flag);
...
static int atbm_ioctl_get_sta_info(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
   ...
   if (!ieee80211_sdata_running(sdata)){
	   atbm_printk_err("atbm_ioctl_get_sta_info:net interface not runing! \n");
	   return  -EINVAL;
   }
   
   if(sdata->vif.type != NL80211_IFTYPE_AP){
	   atbm_printk_err("atbm_ioctl_get_sta_info:interface not run AP mode! \n");
	   return  -EINVAL;
   }   
    ...
    list_for_each_entry_rcu(sta, &local->sta_list, list) {
        ...
        if(sta != NULL){
            if (sta->sdata->vif.type == NL80211_IFTYPE_AP){
                ...
                atbm_printk_always("%d: MAC %pM\n", i, p_info[i].wext_mac);
                atbm_printk_always("    RSSI %d\n", p_info[i].wext_rssi);
                atbm_printk_always("    RX Pkts %ld\n", p_info[i].rx_packets);
                atbm_printk_always("    TX Pkts %ld\n", p_info[i].tx_packets);
                atbm_printk_always("    TX suc Pkts %ld\n", p_info[i].tx_suc_count);
				atbm_printk_always("    RX rate %ld\n", p_info[i].rx_avg_rate);
                atbm_printk_always("    TX rate %ld\n", p_info[i].tx_main_rate);
                atbm_printk_always("    throughput %d byte/s\n", rate_val/16);
                atbm_printk_always("    channel idle %d\n", p_info[i].channel_idle);
                 atbm_printk_always("    \n");
                ++i;
            }
        }
...
static int atbm_ioctl_get_ap_info(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
 ...
 //   atbm_wifi_sta_info_new p_info;
	struct TX_RX_Statistics_S txrx_stat;
	int idle = 0,ret = 0,tx_main_rate;
	char ap_ssid[32] = {0},rssi = 0;
	const char *ssid = NULL;
	unsigned int rate_val;
	if (!ieee80211_sdata_running(sdata)){
		atbm_printk_err("atbm_ioctl_get_ap_info:net interface not runing! \n");
		return  -EINVAL;
	}
	
	if(sdata->vif.type != NL80211_IFTYPE_STATION){
		atbm_printk_err("atbm_ioctl_get_ap_info:interface not run station mode! \n");
		return  -EINVAL;
	}	
	 list_for_each_entry_rcu(sta, &local->sta_list, list) {
	...
	if(!ssid){
		atbm_printk_err("atbm_ioctl_get_ap_info:sta not connect ap! \n");
		return 0;
	}
	ret = atbm_get_txrx_status(hw_priv,(char *)&txrx_stat,sizeof(struct TX_RX_Statistics_S));   
	...
	tx_main_rate = atbm_internal_get_current_txrate(sdata,NULL);
	
	if(ret == 0){
		sprintf(extra,"\nssid=%s\nrssi=%d\nmac:%pM\ntx_pkt=%d\ntx_suc_pkt=%d\nrx_pkt=%d\ntx_rate=%d\nrx_rate=%d\nidle=%d\nthroughput=%d Byte/s\n", 
			ap_ssid,rssi,sta->sta.addr,txrx_stat.transmit_total_cnt,txrx_stat.sum_success_cnt,
			txrx_stat.received_num,tx_main_rate,txrx_stat.rx_rate_avg*10,idle,rate_val/16);
	}else{
		sprintf(extra,"\natbm_ioctl_get_ap_info get fail\n");
	}
...
static int atbm_ioctl_get_snr(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	ret = wsm_get_snr(hw_priv,&snr_val,sizeof(snr_val));
	if(ret == 0){
...
//	sscanf(tim_data,"set_tim,%d,%x",&enable,&tim_bit);
	atbm_printk_err("get data : ena:%d, val:%x i\n",enable,tim_bit);
//	CmdLine_GetHex();
...
{	
	if((etf_config.txpwrmax == 0) && (etf_config.txpwrmin == 0))
	{
		...
		etf_config.txpwrmin = 44;
		atbm_printk_wext("Use default Txrssimin threshold:%d\n", etf_config.txpwrmin);
	}
