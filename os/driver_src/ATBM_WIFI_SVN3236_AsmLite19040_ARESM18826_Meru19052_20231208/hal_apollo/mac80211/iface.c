...
static enum hrtimer_restart ieee80211_napi_watchdog(struct hrtimer *timer)
{
	struct ieee80211_sub_if_data *sdata;

	sdata = container_of(timer, struct ieee80211_sub_if_data, napi_timer);

	if(atbm_skb_queue_len(&sdata->napi_rx_list)){
...
static void ieee80211_sdata_gro_setup(struct ieee80211_sub_if_data *sdata)
{
	...
	if(sdata->vif.type == NL80211_IFTYPE_AP || sdata->vif.type == NL80211_IFTYPE_STATION){
		...
		hrtimer_init(&sdata->napi_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL_PINNED);
		sdata->napi_timer.function = ieee80211_napi_watchdog;
...
static void ieee80211_sdata_gro_disable(struct ieee80211_sub_if_data *sdata)
{
	...
	hrtimer_cancel(&sdata->napi_timer);
...
void ieee80211_sdata_napi_sched(struct ieee80211_sub_if_data *sdata,struct sk_buff *skb)
{
	...
	if(atbm_skb_queue_len(&sdata->napi_rx_list) > 16){
		napi_schedule(&sdata->napi_rx);
	}else {
		hrtimer_start(&sdata->napi_timer, ns_to_ktime(200000),
...
int ieee80211_android_cmd_handle_country(struct ieee80211_sub_if_data *sdata, char *cmd,u32 cmd_len,u8 cmd_index)
{
	struct ieee80211_local *local = sdata->local;
	int ret = 0;
	u8 idx = 0;
	while (reg_regdb[idx]){
		if((reg_regdb[idx]->alpha2[0] == *(cmd + 7 + 1)) &&
			(reg_regdb[idx]->alpha2[1] == *(cmd + 7 + 2))){
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0)
			regulatory_set_wiphy_regd(local->hw.wiphy, reg_regdb[idx]);
#else
			wiphy_apply_custom_regulatory(local->hw.wiphy, reg_regdb[idx]);
#endif
...
static const android_cmd_handle ieee80211_android_cmd_handle_buff[] = {
	...
	ieee80211_android_cmd_handle_default,
	/*ANDROID_WIFI_CMD_COUNTRY*/
	ieee80211_android_cmd_handle_country,
	/*ANDROID_WIFI_CMD_P2P_SET_NOA*/
	ieee80211_android_cmd_handle_default,
	/*ANDROID_WIFI_CMD_P2P_GET_NOA*/
	ieee80211_android_cmd_handle_default,
...
int ieee80211_netdev_ioctrl(struct net_device *dev, struct ifreq *rq, void __user *data,int cmd)
#else
...
#if defined(CONFIG_ATBM_IOCTRL)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
    .ndo_siocdevprivate     = ieee80211_netdev_ioctrl,
#else
