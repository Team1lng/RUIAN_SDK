					     ...
					     struct sk_buff *skb,struct sk_buff_head *frames)
{
	...
	if (seq_less(mpdu_seq_num, head_seq_num)) {
		if(!(status->flag & RX_FLAG_QOS_DUP)){
			ret = false;
			atbm_printk_err("reorder_buf [%d][%d]\n",mpdu_seq_num,head_seq_num);
		}else {
			//atbm_printk_err("reorder_buf dup[%d][%d]\n",mpdu_seq_num,head_seq_num);
			atbm_dev_kfree_skb(skb);
		}
		goto out;
	}
...
static void ieee80211_rx_h_qos_seq(struct ieee80211_rx_data *rx)
{
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)rx->skb->data;
	u16 qos_seq = le16_to_cpu(hdr->seq_ctrl);
	u16 mpdu_seq_num = (qos_seq & IEEE80211_SCTL_SEQ) >> 4;//IEEE80211_SCTL_SEQ = 0xFFF0
	u8  index = mpdu_seq_num % 256;
	struct sta_info *sta = rx->sta;
	/*
		index 每64个包+1
		
		qos_seq 每个包都 +1
		如果缓存里面保存的seq num 和 新包的seq不同 就保存下来。
		如果前面没有过滤掉，进入到这个函数并且新包的seq和上一包的seq相同就设置上dup的标志位，
		再包进入到reoder函数那里的时候，将该包丢掉，并且直接将缓存包上传协议栈。
	*/
...
ieee80211_rx_h_check(struct ieee80211_rx_data *rx)
{
	...
	if (unlikely((ieee80211_is_data(hdr->frame_control) ||
		...
		if (!ieee80211_frame_eapol(rx->skb->data, rx->skb->len)){
			if(rx->sta)
				atbm_printk_err("%s:[%s]:sta[%pM] not assoc[%x]\n",__func__,rx->sdata->name,hdr->addr2,status->flag);
...
ieee80211_rx_h_mgmt_check(struct ieee80211_rx_data *rx)
{
	...
	if (ieee80211_drop_unencrypted_mgmt(rx)){
		atbm_printk_err("unencrypted_mgmt(%x)\n",mgmt->frame_control);
		return RX_DROP_UNUSABLE;
	}
	return RX_CONTINUE;
}
