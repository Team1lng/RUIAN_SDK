	...
	*/
	...
	#ifdef ATBM_CHANGE_AP_TIMESTAMP_TO_BOOTTIME
	{
		...
		if(ieee80211_is_probe_resp(fc)){
			...
			//WARN_ON(1);
			atbm_printk_err("not probe resp & beacon! drop\n");
			return NULL;
		}
	}
	...
	atbm_skb_queue_tail(&sdata->local->scan_rx,skb);
			...
			if(hw_priv->scan.req &&  hw_priv->scan.req->n_ssids > 0){
				if(memcmp(elems.ssid,hw_priv->scan.req->ssids[0].ssid,hw_priv->scan.req->ssids[0].ssid_len) == 0){
					ieee80211_atbm_scan_cancel(&sdata->local->hw);
...
static enum ieee80211_etf_request_action __ieee80211_etf_process_action(struct ieee80211_local *local)
{
	...
	if(local->scan_req_wrap.flags & IEEE80211_SCAN_REQ_ETF_STOP){
		...
		local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_ETF_STARTED;
...
static int __ieee80211_etf_process(struct ieee80211_local *local)
{
	...
	return etf->etf_process(&local->hw,&local->scan_sdata->vif,__ieee80211_etf_process_action(local),&local->scan_req_wrap,NULL);
}
#endif
...
#ifndef CONFIG_ATBM_ETF_OLD
	if(local->scan_req_wrap.flags & IEEE80211_SCAN_REQ_ETF){
		if(__ieee80211_etf_process(local) == 0){
			return;
		}
	}
#endif
...
#ifndef CONFIG_ATBM_ETF_OLD
	if(local->internal_scan.req.req_flags & IEEE80211_INTERNAL_SCAN_FLAGS__ETF_REQUEST){
		local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_ETF;
		local->scan_req_wrap.etf = local->internal_scan.req.etf;
	}
#endif
	local->scan_req_wrap.req = local->hw_scan_req;
	if(ieee80211_scan_policy_prepare(sdata) == true){
