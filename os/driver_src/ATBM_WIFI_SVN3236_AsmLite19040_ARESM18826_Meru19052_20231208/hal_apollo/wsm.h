...
#define WSM_MIB_ID_GET_CCA_THRESHOLD				0x1067 //s32 manual_cca_thr = 0;
#define WSM_MIB_ID_GET_RTS_DURATION					0x1068 //uint32 rtsDuration = 0;
...
#define WSM_MIB_ID_GET_PHY_WRITE_REG_BIT_U32    0x1066//0x1068
#define WSM_MIB_ID_SET_RTS_DUR 					0x1069
#define WSM_MIB_ID_GET_TEMP 				0x1070
#define WSM_MIB_ID_RX_RATE_AVERAGE 			0x1071
...
static inline int wsm_get_txrx_status(struct atbm_common *hw_priv, u8 *txrx_status, int len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_RX_RATE_AVERAGE, txrx_status,
			    len,-1);
}

static inline int wsm_get_temperature(struct atbm_common *hw_priv, u8 *temp, int len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_TEMP, temp,
			    len,-1);
}

static inline int wsm_set_rf_low_power(struct atbm_common *hw_priv, u8 *lowpower, int len)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_RF_LOW_POWER, lowpower,
			    len,-1);
}
...
static inline int wsm_set_tx_precompen(struct atbm_common *hw_priv,void *precomp,int len)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_PRE_COMPENSATION, precomp,
				len, -1);
}
...
typedef struct wsm_phy_init_global_para_s {
	u32 wifi_chip_id;
	u8 rx_outer_lna_flag;
	u8 set_adaptive_flag;
	u16 stationId0[3];
	u16 stationId1[3];
	u8 rfSubtypeDefine;
	u8 aresAy;
	u8 aresAzlc;
	u8 hwChipVersionAresM;
	u8 hwChipVersionAresB;
	u8 hwChipVersionMar;
	u8 customerPlatform;
	u8 sigmastarNvrIpc;
	u8 specialCut;
	...
	u8 chipAresB;
	u8 chipFeatureType;
	u8 richwaveOuterpaSwType;
	u8 kxcomtechOuterpaSwType;
	u8 qorvo4219OuterpaType;
	u8 richwaveOuterpaType;
	u8 reserve[7];
	u32 compileMacro;
