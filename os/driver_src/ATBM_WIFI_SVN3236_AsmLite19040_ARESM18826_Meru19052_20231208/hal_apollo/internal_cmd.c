...
{	
	...
	atbm_direct_read_reg_32(hw_priv, 0xACC017C, &chipID);
	chipID &= 0xff;
	...
	if(chipID == 0x49){
		...
		//--------Asmlite
		//clk_sel:
		HW_WRITE_REG_BIT(0x17400024, 3, 0, 3);
		HW_WRITE_REG_BIT(0x16800028, 16, 16, 0);
		HW_WRITE_REG_BIT(0x16800070, 16, 16, 0);
		pin16 = HW_READ_REG_BIT(0x16800020, 16, 16);

		//gpio22
		HW_WRITE_REG_BIT(0x17400000,8,8,1);//gpio22
		HW_WRITE_REG_BIT(0x17400030, 3, 0, 3);
		...
		HW_WRITE_REG_BIT(0x16800070, 23, 23, 0);
		pin23 = HW_READ_REG_BIT(0x16800020, 23, 23);
		if (pin16)
		{
			etf_config.chip_crystal_type = 1;//gpio 16
		}
		if (pin22)
		{
			etf_config.chip_crystal_type |= BIT(1);//gpio22
		}
		if (pin23)
		{
			etf_config.chip_crystal_type |= BIT(2); //gpio23
		}
	}
...
//	char threshold_param[100] = {0};
	...
	if((chipversion >= 0xA1) && (chipversion <= 0xA3))
		chanMax = 165;//5G max channel num
	...
	else if((is_40M == 2 ) && (channel < 36) && ((channel < 1)||(channel > 11))){
		atbm_printk_err("invalid 40M rate,channel value range:1~11\n");
		return -EINVAL;
	}
	else if((is_40M >= 1 ) && (channel >= 36) && ((channel < 38)||(channel > 161))){
		atbm_printk_err("invalid 40M rate,channel value range:40~161\n");
		return -EINVAL;
	}
	...
	if((rate <= WSM_TRANSMIT_RATE_11)&&(len == 0))
	{
		...
		//			atbm_scan_timeout(&hw_priv->scan.timeout.work);
				...
				if((chipversion == 0x49) || (chipversion == 0x4A)){
					if(((tx_param->precomp_sel == 1) ||  (tx_param->precomp_sel == 2)) && (channel == 13)){
						atbm_printk_always("----SRRC--DSSS--\n");
						hw_priv->etf_rate = WSM_TRANSMIT_RATE_1;//fixed 1M rate
						hw_priv->etf_len = 65535; //fixed packet length
						WARN_ON(wsm_write_mib(hw_priv, WSM_MIB_ID_SET_PRE_COMPENSATION,
							&precomp_sel, sizeof(precomp_sel), vif->if_id));
					}
				}
				hw_priv->etf_rate = 5;//PHY Tx
				if(wsm_start_tx(hw_priv, vif->vif) != 0){
					up(&hw_priv->scan.lock);
					atbm_printk_err("%s:%d,wsm_start_tx error\n", __func__, __LINE__);
					goto _exit;
		...
		if(is_40M == 1)
		{
			if((is_40M == 1 ) && (((channel >= 3) && (channel <= 11))  || ((channel >= 38) && (channel <= 159)))){
				channel += 2;
			}
			is_40M = NL80211_CHAN_HT40MINUS;//center freq=prim chan freq - 10M
		}
		else if(is_40M == 2)
			is_40M = NL80211_CHAN_HT40PLUS;//center freq=prim chan freq + 10M
				...
				else
				{
					...
					if((chipversion == 0x49) || (chipversion == 0x4A)){
						if((tx_param->precomp_sel == 1) || (tx_param->precomp_sel == 2)){
							atbm_printk_always("----SRRC----\n");
							if(channel != 13)
								precomp_sel = 0;
							if(is_40M == 0)
								hw_priv->etf_rate = WSM_TRANSMIT_RATE_54;//SRRC 11n 20M fixed 54M
							else
								hw_priv->etf_rate = WSM_TRANSMIT_RATE_HT_65;//SRRC 11n 40M fixed MCS7
						}
						else if(tx_param->precomp_sel == 3){//CE
							if(rate > WSM_TRANSMIT_RATE_11){//ofdm
								if((is_40M == 0) && ((channel == 1) || (channel == 13)))
									precomp_sel = (channel == 1)?3:4;
								else if((is_40M >= 1) && ((channel == 5) || (channel == 13)))
									precomp_sel = (channel == 5)?3:4;
								else
									precomp_sel = 0;
							}
							else{//dsss
						...
						else if(tx_param->precomp_sel == 4){//FCC
							if(rate > WSM_TRANSMIT_RATE_11){//ofdm
								if((is_40M == 0) && ((channel == 1) || (channel == 11)))
									precomp_sel = (channel == 1)?3:4;
								else if((is_40M >= 1) && ((channel == 5) || (channel == 11)))
									precomp_sel = (channel == 5)?3:4;
								else
									precomp_sel = 0;
							}
...
#endif
int atbm_internal_start_rx(struct ieee80211_sub_if_data *sdata,int channel,int is_40M, int low_power)
{	
	int i = 0;
	char cmd[20] = "monitor 1 ";
	char low_power_cmd[20] = {0};
	u8 ucDbgPrintOpenFlag = 1;
	struct ieee80211_local *local = sdata->local;
	struct atbm_common *hw_priv=local->hw.priv;
	struct atbm_vif *vif;
...
//	memcpy(cmd+10, extra, wrqu->data.length);
	...
	wsm_set_rf_low_power(hw_priv, (u8 *)&low_power, sizeof(low_power));
	sprintf(low_power_cmd, "digital_vol %d", low_power);
	i = 0;
	atbm_for_each_vif(hw_priv,vif,i){
		if (vif != NULL)
		{
			WARN_ON(wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,
				low_power_cmd, 13, vif->if_id));
			break;
		}
	}
...
int atbm_internal_stop_rx(struct ieee80211_sub_if_data *sdata,fixed_freq_rx_data *rx_data)
{
	...
	sprintf(cmd, "digital_vol %d", low_power);
	i = 0;
	atbm_for_each_vif(hw_priv,vif,i){
		if (vif != NULL)
		{
...
int atbm_get_txrx_status(struct atbm_common * hw_priv,char *buf , int buf_len)
{
	...
	struct TX_RX_Statistics_S txrx_status;

	if(!buf || buf_len < sizeof(struct TX_RX_Statistics_S)){
		atbm_printk_err("buff = %x , buf_len=%d \n",buf,buf_len);
		return -1;
	}

	if(wsm_get_txrx_status(hw_priv,(char *)&txrx_status,sizeof(struct TX_RX_Statistics_S)) != 0){
		atbm_printk_err("wsm_get_txrx_status err ! \n");
		return -2;
	}
...
extern void atbm_wifi_insmod_stat_set(int status);
...
static int atbm_ioctl_open(struct inode *inode, struct file *filp)
{
	...
	atbm_printk_err("%s \n", __func__);
#ifdef BLUEDROID
	atbm_printk_err("bluedroid support\n");
	struct ieee80211_local* local;
