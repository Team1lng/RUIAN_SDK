...
void atbm_monitor_pc(struct atbm_common *hw_priv)
{
	...
	atbm_direct_read_reg_32(hw_priv,0x9000000,&val9000000[0]);
	atbm_direct_read_reg_32(hw_priv,0x9000004,&val9000000[1]);
	atbm_direct_read_reg_32(hw_priv,0x9000008,&val9000000[2]);
	...
	atbm_printk_err("ERROR !! pc:[%x],ipc[%x] val9000000[%x][%x][%x]\n",testreg_pc,testreg_ipc,val9000000[0],val9000000[1],val9000000[2]);
...
int atbm_save_efuse(struct atbm_common *hw_priv,struct efuse_headr *efuse_save)
{
	...
	*/
	...
	if (iResult == 0)
	{
		...
		{
			frame_hexdump("efuse_write", (u8 *)&efuse_save, sizeof(struct efuse_headr));
			frame_hexdump("efuse_read", (u8 *)&efuse_bak, sizeof(struct efuse_headr));
			iResult = -2;
		}else
		{
			iResult = 0;
...
void atbm_firmware_cap_int(struct atbm_common *hw_priv)
{
	if(!(hw_priv->wsm_caps.firmeareExCap & EX_CAPABILITIES_DRIVER_PROCESS_BA))
		hw_priv->hw->flags |= IEEE80211_HW_AMPDU_SETUP_IN_HW;
}
