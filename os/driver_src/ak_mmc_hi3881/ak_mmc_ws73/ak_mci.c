...
static int akmci_probe(struct platform_device *pdev)
{
	...
	if(host->gpio_power.gpio >= 0)
	{
			...
			}else
			{
				...
				akmci_pinctrl_idle(host);
				/*2. disable card.*/
				gpio_direction_output(host->gpio_power.gpio, 0);
				msleep(1000);
				gpio_direction_output(host->gpio_power.gpio, 1);
				msleep(500);
				gpio_direction_input(host->gpio_power.gpio);
				msleep(500);
				gpio_direction_output(host->gpio_power.gpio, !host->gpio_power.value);
				pr_info("[%d]GPIO%d with gpio_val:%d\n",__LINE__,host->gpio_power.gpio,gpio_get_value(host->gpio_power.gpio));
				/*3. sleep.*/
				msleep(500);
				/*4. enable card.*/
				gpio_direction_output(host->gpio_power.gpio, host->gpio_power.value);
