...
//hlf
/**
 * @description: 读取指定分区到内存中
 * @param {char} *partition_name	分区名
 * @param {unsigned long} base_addr	内存地址
 * @return {*}
 */
int read_partition_data(char *partition_name, unsigned long base_addr)
{
	...
	int ret = -1;
#endif
	if (get_mtdparts_by_partition_name(partition_name, part_num, &run_add, &run_len) == -1)
	{
