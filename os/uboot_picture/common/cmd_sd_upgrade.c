...
static int upgrade_partitions_update(const struct upgrade_partitions_param *upgrade)
{
    ...
    upgrade_progress_bar_draw(fb_base, 312, 505, 400, 4, 0, 100, 0xFFFFFF, 0xFFFFFF);
    run_command("sf probe", 0);

    for (i = 0; i < UPGARDE_PARTITIONS_NUM_MAX; ++i)
    {
        printf("upgrade->partitions[i].size is %lu\n", upgrade->partitions[i].size);
        printf("upgrade->mtdparts[i].size is %lu\n", upgrade->mtdparts[i].size);
        printf(" %s\n", upgrade->mtdparts[i].name);
        ...
        if (upgrade->partitions[i].size != 0)
        {
            ...
            run_command(cmd, 0);
            upgrade_progress_bar_draw(fb_base, 312, 505, 400, 4, upgrade->partitions[i].weights - upgrade->partitions[i].size / 2, upgrade->upgrade_total, 0xff9326, 0xFFFFFF);
            ...
            run_command(cmd, 0);
            upgrade_progress_bar_draw(fb_base, 312, 505, 400, 4, upgrade->partitions[i].weights, upgrade->upgrade_total, 0xff9326, 0xFFFFFF);
        }
    }
    ...
    for (i = 0; i < 10; i++)
    {
        upgrade_progress_bar_draw(fb_base, 312, 505, 400, 4, 100, 100, 0xff9326, 0xFFFFFF);
        mdelay(500);
        upgrade_progress_bar_draw(fb_base, 312, 505, 400, 4, 100, 100, 0x000000, 0xFFFFFF);
        mdelay(500);
    }
    run_command("reset", 0);
    return 0;
