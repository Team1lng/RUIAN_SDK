...
//hlf
static void jffs2_tmp_cache_init(struct part_info *part)
{
	struct mtdids *id = part->dev->id;
	extern flash_info_t flash_info[];
	flash_info_t *flash = &flash_info[id->num];

	char *saddr = getenv("loadaddr");

	flash->start[0] = saddr ? (ulong)saddr : load_addr;

	if (flash->start[0])
	{
		// printf("========hlf:start:0x%x, offset:0x%x Saddr:0x%p\n", flash->start[0], part->offset,saddr);
		// unsigned long buff = flash->start[0] + part->offset;
		read_partition_data(part->name, (ulong)(flash->start[0] + part->offset));
	}
}


static struct part_info *find_jffs2_part_by_name(char *partition_name)
{
	u8 pnum;
	struct mtd_device *dev;
	struct part_info *part;

	find_dev_and_part(partition_name, &dev, &pnum, &part);
	jffs2_tmp_cache_init(part);
	return part;
}
...
int do_jffs2_fsload(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if ((part = find_jffs2_part_by_name(partitionname)))
	{
...
#else
U_BOOT_CMD(
	fsinfo, 2, 1, do_jffs2_fsinfo,
	"print information about filesystems hlf",
	"partition");
