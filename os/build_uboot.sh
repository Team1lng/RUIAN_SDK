...
install_uboot_image()
{
	...
	if test $FLASH_TYPE = "NAND" ; then
	    ...
	    cp -fv u-boot.bin $uboot_install_dir
	fi
...
build_uboot()
{
	# cyy:7寸ips屏需要在uboot控制lcd复位脚，10寸不需要，所以要添加选项。
	#     新10寸要选3
	
	AK_FB="uboot/drivers/video/ak_fb.c"
	echo "Please select lcd inch type?"
    select LCD in "7INCH" "10INCH" "NEW_10INCH"; 
        do echo "You have selected $LCD"
        break
    done

	if 	 [ "$LCD" = "7INCH" ];  then
        sed -i 's/^#define RESET_INCH_LCD_TYPE.*/#define RESET_INCH_LCD_TYPE 0/' $AK_FB
    elif [ "$LCD" = "10INCH" ]; then
        sed -i 's/^#define RESET_INCH_LCD_TYPE.*/#define RESET_INCH_LCD_TYPE 1/' $AK_FB
	elif [ "$LCD" = "NEW_10INCH" ]; then
	    sed -i 's/^#define RESET_INCH_LCD_TYPE.*/#define RESET_INCH_LCD_TYPE 2/' $AK_FB
...
select_lcd()
{
	lcd="uboot/common/cmd_sd_upgrade.c"
	echo "Please select lcd type?"
	select LCD in "landscape" "portrait"; 
	    do echo "You have selected $LCD"
	    break
	done

	if 	 [ "$LCD" = "landscape" ];  then
		sed -i 's/^#define LCD_SCREEN_LANDSCAPE.*/#define LCD_SCREEN_LANDSCAPE 1/' $lcd
		sed -i 's/^#define LCD_SCREEN_PORTRAIT.*/#define LCD_SCREEN_PORTRAIT 0/' $lcd
	elif [ "$LCD" = "portrait" ]; then
		sed -i 's/^#define LCD_SCREEN_LANDSCAPE.*/#define LCD_SCREEN_LANDSCAPE 0/' $lcd
		sed -i 's/^#define LCD_SCREEN_PORTRAIT.*/#define LCD_SCREEN_PORTRAIT 1/' $lcd
	fi
}
